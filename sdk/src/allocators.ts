...
export class AllocatorVaultClient {
   ...
   */
  async mint(to: string, amount: BigNumber): Promise<ethers.ContractTransaction> {
   ...
   */
  async repay(allocator: string, amount: BigNumber, tokenAddress: string, vaultSpender: string): Promise<ethers.ContractTransaction> {
    const token = new ethers.Contract(tokenAddress, erc20Abi, this.provider) as I0xUSD;

    const allowance = await token.allowance(await (this.provider as ethers.Signer).getAddress(), vaultSpender);
   ...
   */
  async getLine(allocator: string): Promise<{
    ceiling: BigNumber;
    dailyCap: BigNumber;
   ...
   * @param allocator The address of the allocator.
   */
  async getDebt(allocator: string): Promise<BigNumber> {
